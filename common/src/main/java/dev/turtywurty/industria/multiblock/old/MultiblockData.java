...
public record MultiblockData(BlockPos primaryPos, MultiblockType<?> type) {
    public static final Codec<MultiblockData> CODEC = Codec.lazyInitialized(() -> RecordCodecBuilder.create(instance -> instance.group(
            BlockPos.CODEC.fieldOf("primary_pos").forGetter(MultiblockData::primaryPos),
            IndustriaRegistries.MULTIBLOCK_TYPES.get().holderByNameCodec().fieldOf("type").forGetter(data -> IndustriaRegistries.MULTIBLOCK_TYPES.get().wrapAsHolder(data.type()))
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, MultiblockData> STREAM_CODEC = StreamCodec.composite(
            ...
            ByteBufCodecs.holderRegistry(IndustriaRegistries.MULTIBLOCK_TYPES_KEY), data -> IndustriaRegistries.MULTIBLOCK_TYPES.get().wrapAsHolder(data.type()),
