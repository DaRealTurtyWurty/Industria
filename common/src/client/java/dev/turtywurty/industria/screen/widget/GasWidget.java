...
import dev.turtywurty.gasapi.client.GasRenderHandler;
...
public class GasWidget implements Renderable, LayoutElement {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        Gas gas = this.gasTank.getResource().value();
        ...
        long gasCapacity = this.gasTank.capacity(0, this.gasTank.getResource());
    ...
    protected void drawTooltip(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ...
        if (gas != null && gasAmount > 0) {
            context.setComponentTooltipForNextFrame(textRenderer, List.of(
                            gasVariantAttributeHandler.getName(this.gasTank.getResource()),
                            Component.literal((((float) gasAmount / FluidAmounts.BUCKET) * 1000) + " / " + ((int) (((float) gasCapacity / FluidAmounts.BUCKET) * 1000)) + " mB")),
                    mouseX, mouseY);
        }
    }
