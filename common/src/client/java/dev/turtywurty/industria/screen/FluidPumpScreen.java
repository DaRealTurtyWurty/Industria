...
public class FluidPumpScreen extends AbstractContainerScreen<FluidPumpScreenHandler> {
    ...
    public void extractBackground(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        SyncingFluidStorage tank = this.menu.getBlockEntity().getFluidTank();
        Fluid fluid = tank.getResource().value();
    ...
    protected void extractTooltip(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ...
        if (isHovering(146, 8, 16, 60, mouseX, mouseY)) {
            ...
            if (fluid != null && fluidAmount > 0) {
                ...
                context.setTooltipForNextFrame(this.font, Component.literal(((int) (((float) fluidAmount / FluidAmounts.BUCKET) * 1000)) + " / " + ((int) (((float) fluidCapacity / FluidAmounts.BUCKET) * 1000)) + " mB"), mouseX, mouseY + 10);
            }
        }
    }
}
