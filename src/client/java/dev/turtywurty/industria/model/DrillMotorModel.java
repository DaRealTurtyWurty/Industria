...
public class DrillMotorModel extends Model<DrillMotorModel.DrillMotorModelRenderState> {
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        ModelPartData spinRod = main.addChild("spinRod", ModelPartBuilder.create(), ModelTransform.origin(8.2583F, -4.0455F, -0.0001F));
        ...
        ModelPartData rodGear = spinRod.addChild("rodGear", ModelPartBuilder.create().uv(15, 33).cuboid(-7.75F, -0.5F, -0.5F, 10.0F, 1.0F, 1.0F, new Dilation(0.0F))
                .uv(34, 18).cuboid(2.25F, -2.5F, -2.5F, 1.0F, 5.0F, 5.0F, new Dilation(0.0F)), ModelTransform.origin(2.5333F, 0.0F, 0.0F));

        ModelPartData connectingGear = spinRod.addChild("connectingGear", ModelPartBuilder.create().uv(15, 36).cuboid(-0.5F, -2.5F, -2.5F, 1.0F, 5.0F, 5.0F, new Dilation(0.0F)), ModelTransform.origin(5.2833F, 0.0F, 5.0F));
        return TexturedModelData.of(modelData, 64, 64);
    }

    @Override
    public void setAngles(DrillMotorModelRenderState state) {
        super.setAngles(state);
        this.parts.rodGear().pitch = state.clientMotorRotation;
        this.parts.connectingGear().pitch = -state.clientMotorRotation;
    }
